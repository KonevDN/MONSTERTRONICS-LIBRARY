function TagEditor (varName, allTags, selectedTagsId) 
{
	...
	this._suggestionTimeoutHandle = null;
	...
	{
	    window.clearTimeout(this._suggestionTimeoutHandle);
	...
	this.EditorKeyUp = function(event)
	{
	    ...
	    this._suggestionTimeoutHandle = window.setTimeout(new Function(this.VariableName + '.Suggest();'), 249);
	...
	if (this.SelectedTagsHandle)
	{
		...
		this.SelectedTagsHandle.onblur = new Function('event', 'return ' + this.VariableName + '.EditorBlur(event);');
	}
}
...
function InlineTagEditor(varName, clientID, containerId, contentId)
{
	...
	{
	    if (this._callback)
	        this._callback(this._content.value, new Function("result", "eval('result = ' + result); " + this._panelName + ".SetContent(result.tags);"), new Function("alert('An error occurred while saving or formatting tags');"));
